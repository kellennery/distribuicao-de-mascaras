<?php
if(!class_exists('UoDAO')){ require_once 'models/UoDAO.class.php';}
/**
...
class UoController extends Controller{
    ...
    public function __construct(){
        parent::__construct('cadastro-uo');
    }
    ...
    public function listar() {
        ...
        try {
           ...
           // $idUo = self::getVar('idUo');
            $codigo = self::getVar('codigo');
			$sigla = self::getVar('sigla');
			$descricao = self::getVar('descricao');

            $DAO = new UoDAO();
            $listagem = $DAO->listar($page, $rows, $sidx, $sord, null, $codigo, $sigla, $descricao);
            if ($listagem) {
                ...
                foreach ($listagem as $item){
					if ($item->getAtivo()){
						$chkAtivar = '<input type="checkbox" id="ativarUO_'.$item->getIdUo().'" checked onclick=ativarUO('.$item->getIdUo().')>';
					}else{
						$chkAtivar = '<input type="checkbox" id="ativarUO_'.$item->getIdUo().'" onclick=ativarUO('.$item->getIdUo().')>';
					}					
					
                    $this->response->aaData[$i] = array($item->getCodigo(), $item->getSigla(), $item->getDescricao(), $chkAtivar);
                    $i++;
                }
    ...
    public function listarCombo() { 
	   ...
	   try {
			...
			$listagem = $DAO->listar(null, null, 'sigla', 'asc', $idUo, $sigla);
        
			if ($listagem) {
                $this->response->records = count($listagem);
                $i = 0;
                foreach ($listagem as $item){
                    $this->response->rows[$i]['value'] = intval($item->getIdUo()); 
                    $this->response->rows[$i]['text'] = $item->getSigla();
